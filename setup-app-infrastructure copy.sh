...
if ! az account show &>/dev/null; then
    echo "Not logged in to Azure. Please run 'az login' first." >&2
    exit 1
fi
...
register_provider() {
  ...
  >&2 echo "Checking provider registration for: $NAMESPACE"
  local STATE
  ...
  if [ "$STATE" = "Registered" ]; then
    >&2 echo "Provider $NAMESPACE already registered."
    return 0
  fi

  >&2 echo "Registering provider $NAMESPACE..."
  az provider register --namespace "$NAMESPACE" --subscription "$SUB" || true

  local ELAPSED=0
  while [ $ELAPSED -lt $TIMEOUT ]; do
    STATE=$(az provider show --namespace "$NAMESPACE" --subscription "$SUB" --query "registrationState" -o tsv 2>/dev/null || echo "NotRegistered")
    >&2 echo "  $NAMESPACE registration state: $STATE"
    if [ "$STATE" = "Registered" ]; then
      >&2 echo "Provider $NAMESPACE registered successfully."
      return 0
    fi
    sleep $INTERVAL
  ...
  >&2 echo "Timed out waiting for provider $NAMESPACE to register. Current state: $STATE"
  return 1
}
...
for prov in "Microsoft.ContainerInstance" "Microsoft.ContainerRegistry" "Microsoft.Storage"; do
  if ! register_provider "$prov" "$SUBSCRIPTION_ID" 300 5; then
    echo ""
    echo "ERROR: Provider registration failed for $prov. You may need Owner permissions or ask your subscription admin to register it." >&2
    echo "Run: az provider register --namespace $prov --subscription $SUBSCRIPTION_ID" >&2
    exit 1
  fi
done

# -------------------------------------------------------------------
# ACR name availability helper - prints ONLY chosen name to stdout.
# All debug/info goes to stderr (so capturing function output yields just the name).
# -------------------------------------------------------------------
generate_acr_name() {
  local base="$1"
  local sub="$2"
  ...
  for i in $(seq 1 $max_attempts); do
    ...
    if [ "$available" = "true" ]; then
      # Print chosen name to stdout (only this line will be captured by caller)
      printf '%s' "$candidate"
      return 0
    fi
  done
...
ACR_NAME="$(generate_acr_name "$ACR_NAME_BASE" "$SUBSCRIPTION_ID")" || {
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  if docker push "${ACR_NAME}.azurecr.io/${JENKINS_IMAGE_NAME}:${JENKINS_IMAGE_TAG}"; then
    ...
    if [ $RETRY_COUNT -lt $MAX_RETRIES ]; then
      ...
      echo "Failed to push image after $MAX_RETRIES attempts." >&2
      exit 1
    fi
  fi
done
...
JENKINS_URL=$(az container show --resource-group "$RESOURCE_GROUP" --name "$CONTAINER_NAME" --subscription "$SUBSCRIPTION_ID" --query ipAddress.fqdn -o tsv)
...
echo "az container exec --resource-group $RESOURCE_GROUP --name $CONTAINER_NAME --exec-command 'cat /var/jenkins_home/secrets/initialAdminPassword'"